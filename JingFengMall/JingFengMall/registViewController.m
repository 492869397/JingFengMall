...
- (IBAction)confirm:(id)sender {
    if ([self isFillDone]) {
        if ([self.passWord.text isEqualToString:self.passWord2.text]) {
            [self.pass setObject:self.passWord.text forKey:PASSWORD];
            [self.pass setObject:self.sex forKey:SEX];
            [self.pass setObject:self.name.text forKey:@"name"];
            Network *net = [[Network alloc]init];
            [net accessNetUrl:URL_REGISTEINFO parameters:self.pass success:^(NSURLSessionDataTask *task, id responseObject) {
