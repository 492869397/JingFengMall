...
-(void)setOrder:(MallOrderModel *)order
{
    ...
    if (cellSubview) {
        ...
        cellSubview.totalPrice.text = [NSString stringWithFormat:@"共%lu件商品  合计:%@元",(unsigned long)arr.count/2,_order.orderMoney];
        
        NSString *url = [goods.photo stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        [cellSubview.photo sd_setImageWithURL:[NSURL URLWithString:url]];
